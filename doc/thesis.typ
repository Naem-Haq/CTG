...
Sensitivity analyses using alternative grouped seeds remain part of the methodological roadmap and are reported as future work, not as completed inferential evidence in this report. This boundary is stated explicitly to keep validation claims proportional to executed experiments while preserving a clear path for stronger uncertainty quantification in later iterations. This framing matches the final results chapter.
...
Implementation uses Python with a modular `src/` layout (preprocessing, segmentation, feature extraction, CNN, utilities) and notebook blocks for staged experiments and reporting. Core dependencies are NumPy, pandas, SciPy, scikit-learn, WFDB, matplotlib, and PyTorch. Main runs used Linux, Python 3.10.19, CPU execution, and fixed random seeds. Efficiency metrics were measured as wall-clock training duration, per-window inference time, and saved model size. Reproducibility controls include record-level split manifests, JSON/CSV reports, and versioned output artefacts for metrics and figures. Runs are scripted end-to-end and timing logs are retained with artefact exports to support repeatable efficiency comparisons across model families. Shared preprocessing definitions were reused across RF and CNN to avoid implementation drift. Full environment and package versions are listed in Appendix A #cite(<chudacek2014_open>); #cite(<francis2024_scoping>).
...
Interpretation also depends on pipeline policy, not only architecture. Threshold selection, class imbalance handling, and temporal smoothing materially changed operational behaviour, particularly alert stability and escalation burden (Figure @fig:alerts). Benchmark conclusions therefore combine discrimination metrics with deployment-facing indicators, including confusion pathways and post-processing effects.
...
This work contributes an end-to-end CTG benchmarking pipeline that is reproducible, leakage-aware, and clinically interpretable. Methodologically, it provides fair comparison of model families under shared preprocessing, grouped splitting, and unified metric reporting. Empirically, it reports that the implemented RF baseline outperforms the tested 1D-CNN in this data regime (Table @tb:metrics; Figure @fig:rf_cm; Figure @fig:cnn_cm). Practically, it links model evaluation to deployment-facing outputs, including threshold policies, temporal alert behaviour, and auditable artefacts (Figure @fig:alerts; Table @tb:eff_explicit). It also provides reusable reporting templates and artefact structures that lower replication barriers for future cohorts and follow-on validation work #cite(<feng2023_shap>).
...
Use the Random Forest pipeline as the reference model for clinical decision support experiments, with thresholds tuned to stated safety objectives and reviewed by stakeholders (Table @tb:metrics; Figure @fig:rf_cm).
...
Prioritize explainable output presentation in user interfaces, including probability context and top contributing feature domains, so recommendations remain interpretable at the point of care and support transparent clinician override (Figure @fig:rf_fi).
