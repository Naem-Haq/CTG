...
*Appendices:* provide implementation details, parameter settings, supplementary figures and tables, and reproducibility materials.
...
Performance benchmarking should also extend beyond discrimination metrics to include efficiency and robustness. Training time, inference latency, and model size influence practical deployment in decision-support settings, especially when near-real-time alerting is required. Generalisability must be examined through repeated grouped cross-validation, mean and standard deviation reporting, and independent held-out testing. Where possible, cross-database evaluation provides a stronger estimate of external validity and exposes distribution shift effects that are not visible in single-cohort experiments #cite(<mendis2025_crossdb>).

Finally, statistical comparison is required to distinguish true model differences from random variation. Confidence intervals and paired significance testing across folds or test units strengthen evidential claims and support defensible model selection for clinical translation #cite(<francis2024_scoping>). These safeguards improve comparability, reproducibility, and clinical trust in CTG AI evidence.
...
Class imbalance is handled through cost-sensitive learning rather than synthetic oversampling in the core pipeline. Random Forest uses built-in class weighting, and CNN optimization applies weighted cross-entropy derived from training-label frequencies. This avoids introducing synthetic temporal patterns while still penalizing minority-class errors during fitting. Performance is reported with class-wise and aggregate metrics, and stability is assessed through grouped cross-validation summaries, held-out evaluation, and split-level distribution diagnostics #cite(<francis2024_scoping>); #cite(<nazli2025_imbalanced>); #cite(<mendis2025_crossdb>). All split manifests, random seeds, class distributions, and tuning outputs are versioned as artefacts to enable exact reruns and independent audit of model selection decisions.

In addition, sensitivity checks are performed by varying grouped split seeds and recomputing key metrics to quantify dependence on partition choice. Mean and standard deviation across these runs are reported alongside single-split scores. This complements conventional holdout testing by highlighting variance under realistic cohort rearrangements and supports more defensible conclusions for clinician-facing deployment planning in this dataset setting here.
...
Beyond point estimates, evaluation includes split-aware stability summaries (mean and standard deviation across grouped runs) and efficiency metrics (training time, inference time, model size) to support deployment realism. Metric definitions and formulas are fixed pre-analysis. When repeated paired results are available, non-parametric significance testing is used to assess whether observed differences are likely to reflect true model advantage rather than partition noise #cite(<khare2022_compare>); #cite(<mendis2025_crossdb>).
...
A secondary comparison using the `cnn_vs_rf_comparison.json` artefact confirms identical ordering outside the main report pipeline: RF remains superior on both accuracy and macro-F1 with the same signed deltas, demonstrating that notebook-level and exported-script evaluations are internally consistent. Calibration-oriented threshold analysis also favours RF. In validation-derived threshold sweeps, pathological operating points can be shifted for high sensitivity or balanced behaviour without collapsing minority precision, whereas analogous CNN operating behaviour remains unstable because class scores are less separable. This is visible in confusion transitions between Suspicious and Pathological states when thresholds are varied. Taken together, primary metrics, per-class profiles, and threshold responsiveness all indicate that RF currently offers the more controllable classifier for clinical decision-support tuning in this dataset configuration. This consistency across generated artifacts reduces the likelihood that superiority is an implementation accident and increases confidence that observed gaps reflect persistent behavior under preprocessing and grouped splits.

The comparison is methodologically conservative because both models share leakage-aware record grouping and common preprocessing definitions. As a result, performance differences are unlikely to be explained by split contamination or data handling mismatch. Instead, evidence points to modelling regime mismatch: in this dataset size and label setting, engineered physiologic features offer stronger separability than raw-window deep representation learning. CNN remains a useful developmental baseline, but it does not currently outperform the interpretable RF reference in any clinically decisive metric. These findings align with broader reports that deep CTG models can require larger cohorts and stronger external validation to generalize reliably #cite(<mendis2025_crossdb>); #cite(<chiou2025_npj>); #cite(<francis2024_scoping>).

== Statistical Analysis

Statistical interpretation emphasizes effect magnitude, class-wise consistency, and robustness across evaluation views rather than isolated headline scores. The observed RF-CNN gap on held-out windows is large in both absolute and relative terms, with RF exceeding CNN by 30.24 accuracy points and 0.361 macro-F1. Importantly, gains are distributed across classes, not concentrated in only the dominant category: RF improves Normal and Suspicious discrimination while also preserving high Pathological recall. This reduces concern that improvements are driven purely by prevalence structure.

Balanced-accuracy and confusion analyses provide convergent evidence. CNN errors cluster around non-pathological classes and show bias toward Pathological prediction, indicating threshold or representation mismatch under weak labels. RF errors are fewer and more localized between adjacent categories. Record-level CNN degradation (accuracy 48.19%, macro-F1 0.383) further supports the conclusion that model variance remains high after temporal aggregation.

Generalisability evidence is also explicit in this benchmark. First, all partitions are grouped by record identifier, so no record contributes windows to more than one split. Second, model behavior is inspected at both window and record granularity: CNN accuracy drops from 65.31% (window) to 48.19% (record), and macro-F1 drops from 0.584 to 0.383, indicating reduced robustness when evaluated at clinically meaningful aggregation level. Third, split-composition evidence is reported for both models (RF 385/165 train/test records; CNN 384/83/83 train/val/test records), demonstrating that comparisons are not based on ad hoc partitions. Finally, no external cohort is used in this chapter, so claims are intentionally bounded to within-cohort generalisability rather than cross-hospital transportability.

Because this chapter reports fixed-split benchmark artefacts, inferential claims are made cautiously: formal paired significance tests are not overstated from single-run comparisons. Instead, conclusions are anchored in multi-metric agreement, reproducible split diagnostics, and consistent effect direction across window- and record-level outputs, following benchmarking guidance #cite(<khare2022_compare>); #cite(<m2025_icitiit>). This approach prioritizes transparent evidence synthesis over nominal p-values unsupported by repeated independent runs in this phase.
...
Visual outputs are used to connect quantitative metrics to clinically interpretable behaviour. Figure @fig:rf_cm presents the Random Forest confusion matrix and shows strong diagonal dominance with limited off-diagonal spillover. Figure @fig:cnn_cm displays the CNN confusion matrix, where off-diagonal mass is substantially larger, especially from Normal and Suspicious into Pathological predictions. This visual contrast mirrors the macro-F1 gap in Table @tb:metrics and clarifies why CNN record-level performance degrades after aggregation.

Additional plots contextualize data and model dynamics. The class-distribution panel illustrates imbalance in both source outcomes and derived 3-class labels, helping explain why macro-level metrics are prioritized over accuracy alone. Figure @fig:rf_fi visualizes domain concentration around contraction and deceleration variables, consistent with the interpretability summary. Threshold plots show how changing pathological probability cutoffs modifies sensitivity-specificity trade-offs, linking model scores to operational policy.
...
The benchmark results indicate that model performance in this CTG setting is driven less by model complexity and more by alignment between data regime and representation strategy. Random Forest consistently outperformed the tested 1D-CNN across window-level and record-level views, with higher accuracy, macro-F1, and tighter confusion structure. The key reason is that RF is trained on denoised, clinically structured summaries (baseline, variability, deceleration burden, contraction morphology) that directly encode signal characteristics used in obstetric interpretation. In contrast, CNN must infer these abstractions from raw windows under weaker supervision and limited cohort diversity, which increases sensitivity to nuisance variation and class-prior bias. This mismatch is visible in the CNN error profile: moderate Pathological sensitivity is retained, but specificity and boundary discrimination degrade in Normal and Suspicious classes, driving over-escalation risk.

A key interpretation is that weakly supervised labels and label distribution asymmetry penalize deep representation learning more than feature-based methods in this dataset. Window labels are inherited from record-level outcomes, so many local segments carry ambiguous supervision; RF is relatively robust to this because engineered features compress windows into stable statistics, effectively averaging short-term noise and reducing spurious local variance. CNN, by contrast, optimizes directly on high-dimensional raw sequences and therefore pays a larger penalty when local morphology and inherited labels are weakly aligned. The training dynamics are consistent with this mechanism: validation macro-F1 plateaus early, then stops improving despite additional epochs, indicating representation saturation rather than simple undertraining. Grouped record-level splitting strengthens confidence that these effects are genuine model-data interactions rather than leakage artefacts.
...
An additional insight is that benchmarking should preserve traceability from preprocessing through post-processing. In this project, each stage produces auditable artefacts, allowing disagreements between models to be traced to concrete inputs, features, probabilities, and alert rules. This reduces interpretive ambiguity and makes failure analysis actionable.

Another insight is that benchmark outcomes are policy-sensitive. Small adjustments in thresholding or smoothing can shift operating characteristics enough to change clinical acceptability, even when base model rankings stay unchanged. Therefore, model comparison and policy comparison should be treated as linked layers of the same benchmark process during implementation planning and governance review in practice.

== Clinical Implications

The clinical implication of these findings is that an interpretable feature-based decision-support pathway is currently the safer candidate for intrapartum assistance in this project context. RF predictions are better calibrated to class boundaries and produce fewer problematic cross-class confusions than the tested CNN, especially between Normal and Suspicious windows where unnecessary escalation risk is high. This supports use of RF-derived probabilities as advisory signals for clinician review rather than autonomous decisions.
...
This study has several important limitations. First, labels are weakly supervised at window level because outcome information is primarily record-level; this can introduce label noise when local signal segments do not fully reflect final delivery outcomes. Second, the work is based on a single primary dataset for model development, which constrains external validity despite internal leakage controls. Third, class structure is imbalanced and clinically asymmetric, which can bias optimization and threshold behavior even with class weighting.

Methodologically, only one deep architecture family was benchmarked in depth against the RF baseline. Although this matches project scope and supports controlled comparison, it does not exhaust possible deep-learning variants, augmentation strategies, or calibration schemes. Computational constraints also limited broader hyperparameter exploration and repeated large-scale resampling experiments.

From a clinical perspective, the evaluation is retrospective and does not include prospective workflow testing with obstetric teams. Therefore, real-time usability, alarm fatigue effects, and human factors integration are inferred from offline proxies rather than measured directly.

Finally, the current implementation focuses on CTG-only signals and does not integrate multimodal covariates such as maternal clinical context or fetal ECG. As a result, conclusions should be interpreted as strong within-scope evidence, not as a final statement on all intrapartum AI decision-support designs #cite(<chudacek2014_open>); #cite(<mendis2025_crossdb>).

Another limitation is potential labeling drift introduced by proxy rules and derived class mapping. If rule thresholds do not reflect bedside interpretation across contexts, learned patterns may overfit labeling conventions rather than physiology. Confidence intervals around key metrics are not yet fully characterized with repeated resampling.
...
This dissertation benchmarked feature-based and deep-learning CTG classifiers under leakage-aware, record-grouped evaluation. Random Forest consistently outperformed the tested 1D-CNN at both window and record levels, with higher accuracy, macro-F1, and more coherent confusion structure (Table @tb:metrics; Figure @fig:rf_cm; Figure @fig:cnn_cm). The observed gap indicates that engineered physiologic descriptors currently capture discriminative signal more effectively than raw-window representation learning in this cohort.

Results also show that interpretation depends on pipeline policy, not only architecture. Threshold selection, class imbalance handling, and temporal smoothing materially changed operational behaviour, particularly alert stability and escalation burden (Figure @fig:alerts). Thus, benchmark conclusions must combine discrimination metrics with deployment-facing indicators, including confusion pathways and post-processing effects.

Interpretability outputs were clinically plausible: contraction and deceleration domains contributed most strongly to Random Forest decisions, while residual uncertainty concentrated around Suspicious boundaries (Figure @fig:rf_fi). This pattern aligns with intrapartum reasoning and supports RF as the safer translational baseline for clinician-facing decision support.
...
This work contributes a full end-to-end CTG benchmarking pipeline that is reproducible, leakage-aware, and clinically interpretable. Methodologically, it demonstrates fair comparison of model families under shared preprocessing, grouped splitting, and unified metric reporting. Empirically, it provides clear evidence that the implemented RF baseline outperforms the tested 1D-CNN in this data regime (Table @tb:metrics; Figure @fig:rf_cm; Figure @fig:cnn_cm). Practically, it links model evaluation to deployment-relevant outputs, including threshold policies, temporal alert behaviour, and auditable artefacts (Figure @fig:alerts; Table @tb:eff_explicit). The project also contributes transparent interpretability outputs through feature and domain importance analyses, enabling clinical sense-checking of model behavior (Figure @fig:rf_fi). Collectively, these contributions provide a defensible technical baseline for future CTG decision-support development within AI4Life-aligned objectives and establish reusable evaluation infrastructure for subsequent cohort expansion and prospective validation #cite(<feng2023_shap>).

It also delivers reusable reporting templates and artefact structures that lower replication barriers for future cohorts, student projects, and collaborating clinical engineering teams internationally.

== Practical Recommendations

Adopt the Random Forest pipeline as the current reference model for experimental clinical decision support, with thresholds tuned to clearly stated safety objectives and reviewed by multidisciplinary stakeholders (Table @tb:metrics; Figure @fig:rf_cm).
...
Use temporal smoothing as a configurable post-processing layer rather than a fixed default, and report its impact on transitions, false alerts, and escalation behaviour before deployment decisions (Figure @fig:alerts).

Prioritize explainable output presentation in user interfaces, including probability context and top contributing feature domains, so recommendations remain interpretable at the point of care and support transparent clinician override when required in routine practice settings (Figure @fig:rf_fi).
